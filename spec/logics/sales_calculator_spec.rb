...
describe SalesCalculator do
  ...
  describe '#summary_with_rate_for_transition' do
    before do
      ...
      @prev_year_summaries = @summaries.map.with_index {|s, i|
    ...
    it do
      summaries = SalesCalculator.summary_with_rate_for_transition(@places[0], Date.new(2017, 7, 1)).to_a
      expect(summaries.count).to eq(12)
      expect(summaries.last.place.id).to eq(@places[0].id)
      expect(summaries.last.target_month).to eq('201707')
      expect(summaries.last.sales_count).to eq(1*12)
      expect(summaries.last.sales_count_prev_year_rate).to eq(0.1)
      expect(summaries.last.sales_count_prev_month_rate).to eq(12.0 / 11.0)
      expect(summaries.last.sales_amount).to eq(100*12)
      expect(summaries.last.sales_amount_prev_year_rate).to eq(0.5)
      expect(summaries.last.sales_amount_prev_month_rate).to eq(12.0 / 11.0)

      expect(summaries.first.place.id).to eq(@places[0].id)
      expect(summaries.first.target_month).to eq('201608')
      expect(summaries.first.sales_count).to eq(1)
      expect(summaries.first.sales_count_prev_year_rate).to eq(0.1)
